...
public:
        ...
        static HROVSettingsPtr BuildHROVSettings(const merbots_whrov_msgs::hrov_settings::ConstPtr & cstate)
        {
...
private:
        ...
        void _SetROIConf(uint16_t x0 = 0, uint16_t y0 = 0, uint16_t x1 = 0, uint16_t y1 = 0, uint8_t shift = 0);
	...
	void _SetImgResolution(uint16_t resolution);
        ...
        void _GetROIConf(int16_t & x0, int16_t & y0, int16_t & x1, int16_t & y1, uint8_t & shift);
	uint16_t _GetROIX0();
	uint16_t _GetROIY0();
	uint16_t _GetROIX1();
	uint16_t _GetROIY1();
